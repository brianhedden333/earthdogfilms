...
const VideoLightbox = ({ isOpen, onClose, vimeoId, vimeoHash, youtubeId, title }: VideoLightboxProps) => {
  const getEmbedUrl = () => {
    if (vimeoId) {
      const hashParam = vimeoHash ? `&h=${vimeoHash}` : '';
      return `https://player.vimeo.com/video/${vimeoId}?autoplay=1${hashParam}`;
    }
    if (youtubeId) {
